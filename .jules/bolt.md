...
## 2026-05-28 - Documentation Directory Discovery Optimization
**Learning:** Redundant filesystem scans for documentation directory discovery can be eliminated by caching the result in a module-level variable. Additionally, using a pre-allocated array for promises instead of `.map(async ...)` reduces memory overhead and garbage collection pressure in hot paths or frequently used tools.
**Action:** Implemented `cachedDocsDir` and refactored `getDocsDir` to use a pre-allocated promise array in `src/tools/composite/help.ts`.
