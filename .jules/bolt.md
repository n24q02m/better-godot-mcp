...
## 2025-05-15 - Indexing Node Types for Faster Filtering
**Learning:** Iterating over all nodes in a large Godot scene to filter by type (e.g., finding all UI Controls) results in O(N) performance per request. For massive scenes with 20k+ nodes where only a small fraction matches the filter, this creates unnecessary overhead.
**Action:** Introduced a `nodesByType: Map<string, SceneNodeInfo[]>` index in the core `ParsedScene` structure. Populating this index during the initial single-pass parse allows $O(C)$ lookup for specific types (where $C$ is the number of control types), significantly reducing the search space and improving request latency by ~50% in benchmarks.
