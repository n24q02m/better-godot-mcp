...
## 2025-05-15 - [PERF] O(N^2) array includes inside loop in detector.ts
**Learning:** Performing multiple passes over an array with regex tests and string includes inside a loop can be optimized by using a single pass with fast string short-circuits (`startsWith`, `endsWith`). Pre-compiling regexes and Buffers at the module level also avoids redundant memory allocations and compilation overhead.
**Action:** Refactored `findWinGetGodotBinaries` in `src/godot/detector.ts` to use a single `for...of` loop with string-based short-circuits and pre-compiled regex constants. Moved binary signatures to module-level `Buffer` constants in `isLikelyGodotBinary`.
