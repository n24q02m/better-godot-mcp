...
## 2024-07-24 - TypedArray Bigram Encoding for Fuzzy Search
**Learning:** In highly trafficked fuzzy search fallback paths (like `findClosestMatch`), generating string bigrams and inserting them into standard Javascript `Set` objects causes significant garbage collection overhead. Furthermore, attempting to use bitwise operators `(char1 << 8) | char2` to tightly pack strings into integers works for ASCII but creates collisions for 16-bit Javascript characters (UTF-16).
**Action:** Use pre-allocated `Float64Array` instances and encode bigrams via mathematical shifting (`char1 * 65536 + char2`) to safely and efficiently track character pairs without allocating intermediate strings or hitting the 32-bit bitwise operator ceiling. This reduces execution time substantially in hot paths.
