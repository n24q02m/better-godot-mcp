...
## 2026-07-26 - [Optimize high-throughput stream log parsing]
**Learning:** For high-throughput stream processing functions (e.g., `pushLog` in `src/godot/headless.ts`), using `chunk.toString().split(/\r?\n/)` introduces unnecessary array allocations (including empty strings for consecutive newlines) and regex execution overhead. This increases garbage collection pressure, especially for chatty logging output.
**Action:** Optimize by replacing the `split` call with a fast `while` loop that uses `indexOf('\n', pos)` and `slice(pos, lineEnd)`. This manual traversal completely avoids intermediate array creation and regex overhead, significantly reducing memory churn in Node.js streams.
