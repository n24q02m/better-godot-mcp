...
## 2026-06-26 - [Optimize Path Normalization String Parsing]
**Learning:** Using regular expressions followed by string splitting, array filtering, and joining (e.g., `.match().split().filter().join()`) for simple path manipulation introduces unnecessary intermediate arrays and RegExp execution overhead, which can become a bottleneck in heavily used utility functions.
**Action:** Replace RegExp and array allocation chains with fast-path string methods (`toLowerCase()`, `startsWith()`, `indexOf()`, and `slice()`) to achieve zero-allocation parsing for hot path operations like stripping root prefixes.
