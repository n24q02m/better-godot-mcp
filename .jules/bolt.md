...
## 2025-07-20 - [Avoid array slice allocations in array parsing]
**Learning:** Slicing a string to parse an array's inner contents (e.g., `const inner = str.slice(1, -1)`) inside a parsing loop allocates an entirely new string in memory. When dealing with heavily nested or numerous arrays (like those seen in Godot files), these intermediate allocations create severe garbage collection pressure and slow down parsing significantly.
**Action:** Replaced string slicing in `parseGodotValue` with direct start/end indices acting on the original string, significantly improving structural parsing performance in high-frequency paths.
