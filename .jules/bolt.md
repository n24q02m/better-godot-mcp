...
## 2026-07-20 - [Optimize node path normalization]
**Learning:** In the scene node path normalizer (`normalizeNodePath`), extracting segments with regular expressions (`ROOT_PATH_REGEX.exec`) and chaining array allocation methods (`split('/').filter(Boolean)`, `.slice(1).join('/')`) causes significant and unnecessary performance overhead, especially in frequently executed path-resolution tasks.
**Action:** Replaced regular expression and array-based normalization in `normalizeNodePath` with direct string manipulation via index tracking (e.g. `normalized.charCodeAt`, `indexOf`, and `slice`) to process slashes without dynamic array allocations.
