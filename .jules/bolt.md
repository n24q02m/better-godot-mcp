...
## 2025-01-24 - Node Path Normalization
**Learning:** Godot node paths can be represented in multiple ways by LLMs (absolute with `/root/`, relative with `./`, GDScript accessors like `$`, or wrapped in `NodePath()`). Strict normalization must handle these to ensure tool reliability. However, special care must be taken for the case-sensitivity of the scene root node name (e.g., "Root" vs "root") when it's NOT part of an absolute path prefix.
**Action:** Use a multi-stage normalization pipeline that strips wrappers, handles mistaken resource prefixes, collapses redundant slashes, and applies context-aware root stripping (case-insensitive for `/root/` prefixes, but preserving standalone node names unless they are explicitly `/root`).
