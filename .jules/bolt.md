...
## 2025-06-25 - [Pre-allocate arrays for filtered node mapping]
**Learning:** Using `Array.prototype.push()` inside a loop that maps subsets of nodes (such as building `controls` or `players` from `nodesByType`) causes V8 to dynamically resize the array under the hood. While fast, for large or deeply nested scenes where mapping is a hot path, this overhead accumulates.
**Action:** Always pre-calculate the required array size (e.g., by summing `typeNodes.length` or using the length directly) and use `new Array(size)` with indexed assignment instead of `.push()`.
