...
## 2025-05-15 - [PERF] Optimized documentation directory discovery
**Learning:** Using `Promise.all` with `Array.map` for sequential-like file existence checks creates unnecessary arrays and triggers parallel I/O operations even when the first candidate might match. For discovery logic where the first success is sufficient, a sequential `for...of` loop is more efficient and reduces garbage collection pressure.
**Action:** Replaced `Promise.all` with a sequential loop in `src/tools/composite/help.ts` and implemented module-level caching for the discovered directory to eliminate redundant filesystem hits in subsequent calls.
