...
## 2025-06-25 - [Optimize exact match guards in normalizeNodePath]
**Learning:** For exact string matching (like checking for `/root`), it's faster to do a `.length` check before doing `.toLowerCase()`. Also, when stripping characters like backslashes, guarding `.replaceAll('\\', '/')` with an `.includes('\\')` check avoids unnecessary allocations and operations when the string doesn't contain the character. However, rewriting `split('/').filter(Boolean).join('/')` with a manual string traversal loop sacrifices code readability for negligible micro-optimization gains, directly violating project guidelines.
**Action:** Always guard `replaceAll()` operations with an `.includes()` check. When checking an exact string match case-insensitively, use `.length === N` before `.toLowerCase() === 'match'`. Retain declarative array methods (`split().filter().join()`) in less frequent paths if replacing them results in unreadable code.
