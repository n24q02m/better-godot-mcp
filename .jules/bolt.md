...
## 2025-06-25 - [Anti-Pattern] Over-optimizing Cold Paths (Fuzzy Matching)
**Learning:** Using complex bitwise operations and mathematical bigram encoding (`Uint32Array` intersections) for string suggestions in error handlers (e.g., `findClosestMatch`) is a classic anti-pattern. This is a cold path only invoked when users type invalid actions. The micro-optimization heavily sacrifices readability and maintainability for negligible real-world gains, and can easily introduce subtle regressions (like breaking single-character edge cases) that compromise the utility of the helper.
**Action:** Do not attempt to optimize cold paths or error-handling suggestions. Prioritize code readability, correctness, and safety over raw execution speed unless the function is proven to be a bottleneck via profiling.
