...
## 2025-06-25 - [Optimize regex replacement with string traversal in hot paths]
**Learning:** Using regular expressions like `/^\/?root\/(.+)$/i` alongside array allocation methods (`split`, `filter`, `join`) for frequent string normalization tasks (like `normalizeNodePath` called for every node) adds significant garbage-collection overhead and regex execution cost. Replacing these with manual string traversal and fast-path string methods (`charCodeAt`, `startsWith`, `indexOf`) eliminates intermediate allocations and runs significantly faster.
**Action:** Replace regular expressions and array allocation methods (`split`, `filter`, `join`, `replace`) with manual string traversal and fast-path string methods (`indexOf`, `replaceAll`, `startsWith`, `charCodeAt`) when performing frequent string parsing tasks, such as path normalization in `src/tools/composite/nodes.ts`.
