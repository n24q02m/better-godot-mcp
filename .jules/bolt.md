...
## 2025-05-15 - [Cleanup] Optimization of Physics Tool
**Learning:** Using `String.prototype.match()` with capturing groups on large file contents in hot paths causes unnecessary array object allocations. Reusing established line-by-line transformation utilities like `updateNodeInScene` provides a more robust and performant way to modify Godot scene files while avoiding manual Regex management and redundant `escapeRegExp` calls.
**Action:** Refactored `collision_setup` and `body_config` in `src/tools/composite/physics.ts` to utilize `updateNodeInScene`, removing manual Regex matching and string slicing.
