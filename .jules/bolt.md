...
## 2026-05-28 - Refactored parseTscnFile to reduce nesting
**Learning:** Extracting parsing logic into small functional units improves readability without sacrificing the performance of direct string traversal.
**Action:** Refactored `parseTscnFile` in `src/tools/composite/scenes.ts` by extracting `tryParseNode` and `tryParseScript` and using guard clauses.
