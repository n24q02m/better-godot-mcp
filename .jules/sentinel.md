...
## 2025-05-27 - Implicit Array Coercion Bypass
**Vulnerability:** String validation logic checking `typeof val === 'string'` can be bypassed by array inputs (`['malicious\ncode']`). Arrays implicitly coerce to strings during template interpolation, bypassing the validation and resulting in arbitrary injection into Godot text files.
**Learning:** Checking the type of an input via `typeof` in JavaScript is insufficient when the input might be an array (which has `typeof` as `'object'`) and is subsequently interpolated or coerced into a string.
**Prevention:** Explicitly coerce unknown/untyped variables using `String(val)` prior to validation instead of relying on `typeof` checks to ensure all interpolated forms of the data are securely validated.
