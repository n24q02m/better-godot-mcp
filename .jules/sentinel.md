...
## 2025-02-27 - Fix Type Confusion in validateNoNewlines
**Vulnerability:** The `validateNoNewlines` function used `typeof val === 'string'` before checking for newline characters. This allowed an attacker to pass an array (e.g. `["value\n"]`) to bypass validation, which could later be implicitly coerced into a string containing the newline payload (e.g. during string interpolation for Godot files).
**Learning:** Type checking via `typeof` is insufficient for validating untyped JSON payloads against string-based injection attacks, as non-strings can be coerced to strings *after* validation.
**Prevention:** When checking for string-based vulnerabilities on unknown inputs, explicitly coerce the input to a string (`String(val)`) before applying validation checks like `.includes()`.
