...
## 2024-03-24 - Missing Input Coercion for GodotMCP properties parameter
**Vulnerability:** Node property values given by LLMs were bypassing string sanitization when passed in as an array of strings within the `properties` argument object because they lacked `String(k)` and `String(v)` coercion.
**Learning:** `typeof` validation is not sufficient when the user supplies malicious untyped arrays that implicitly coerce via JS template string interpolation during injection but bypass initial `includes` sanity checks since they're not originally strings.
**Prevention:** Explicitly coerce object entries to `String` and validate untyped JSON using string coercion where necessary for dynamic template building like `.tscn` files.
