...
## 2026-07-31 - Prototype Pollution in Template Lookups
**Vulnerability:** Untrusted user input was used as a key to access plain objects (like `CONTROL_TEMPLATES` and `SHADER_TEMPLATES`) without prototype checks.
**Learning:** Just like action dispatchers, plain object lookups used for template configuration expose the server to prototype pollution attacks. If an attacker supplies a template type matching an `Object.prototype` key (like `toString`), it can cause unintended behavior or errors.
**Prevention:** Always wrap key lookups on unverified user input against plain object maps using `Object.hasOwn(obj, key)` before accessing the values.
