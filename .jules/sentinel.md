...
## 2025-02-24 - [Path Traversal in Handlers Using SafeResolve Ad-Hoc]
**Vulnerability:** Path traversal vulnerabilities allowed reading, writing, or deleting files outside the project by exploiting handlers that manually invoked `safeResolve(base, args.project_path)`. While `safeResolve` verifies containment within the given base, several tools (like `editor`, `scripts`, and `nodes`) allowed an attacker-controlled `project_path` to escape the root boundary if the base was not strictly enforced first.
**Learning:** `safeResolve` is not sufficient if the `base` it uses is an unvalidated combination of `config.projectPath` and untrusted input. The repository explicitly provides a `resolveProjectRoot(args.project_path, config.projectPath)` helper designed to confine the user-provided `project_path` safely within `config.projectPath` before any per-file resolutions occur.
**Prevention:** Always use `resolveProjectRoot(args.project_path, config.projectPath)` at the top level of a tool handler to establish a trusted project root before doing any file path derivations. Never manually attempt to sanitize `project_path` with ad-hoc `safeResolve` calls.
