...
## 2024-05-18 - [Fix validateNoNewlines Array Bypass]
**Vulnerability:** Newline injection check (`validateNoNewlines`) could be bypassed by passing an array or object containing newlines (e.g. `["my_anim\"\n[node name=\"HACKED\"]"]`), because it only checked `if (typeof val === 'string')`. This bypassed the check entirely, and the array was later implicitly converted to a string when concatenated into Godot text files (.tscn, .tres), resulting in injection.
**Learning:** Type checking (`typeof val === 'string'`) is insufficient at security boundaries when the data will eventually be coerced to a string. Attackers can use arrays or objects to bypass the check.
**Prevention:** Always stringify arguments at the security boundary before performing checks (e.g. `String(val).includes('\n')`), ensuring the check runs on the exact representation that will be used downstream.
