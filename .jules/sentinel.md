...
## 2025-05-22 - [Array Coercion Vulnerability]
**Vulnerability:** Untyped JSON parameters (such as `args.name`) were coerced to strings using TypeScript's `as string` and then validated with `.includes()`. An attacker could pass an array of strings (e.g., `["malicious\ncode"]`), which bypassed the exact-match `.includes()` check on the array, but was later implicitly coerced into a malicious string during template interpolation.
**Learning:** TypeScript type assertions (`as string`) provide no runtime validation. Methods like `.includes()` behave differently on arrays than on strings, allowing security checks to be bypassed if the type is unexpected.
**Prevention:** Always explicitly coerce untyped external inputs to strings using `String(value)` before performing string-specific validation like `.includes()`.
