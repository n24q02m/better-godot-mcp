...
## 2024-05-27 - [File Injection via Untyped Parameter Interpolation]
**Vulnerability:** In `src/tools/composite/navigation.ts`, numeric and boolean parameters intended for generated Godot scenes (`radius`, `max_speed`, `path_desired_distance`, `target_desired_distance`, `avoidance_enabled`) were interpolated directly into node definitions without explicit type checks.
**Learning:** TypeScript `as number` or `as boolean` assertions do not enforce runtime types. If an attacker provided a crafted string (e.g., containing newlines and `[node name="Malicious" type="Node"]`), it would be successfully injected into the Godot file, potentially leading to arbitrary code execution or scene corruption upon loading.
**Prevention:** Always perform runtime `typeof` validation (e.g., `typeof arg !== 'number'`) on arguments passed via the MCP tool interface before interpolating them into Godot configuration or scene files.
