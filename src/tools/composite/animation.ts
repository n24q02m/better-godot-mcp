...
async function createPlayer(projectPath: string | undefined, args: Record<string, unknown>) {
  ...
  try {
    validateNoNewlines(undefined, playerName, parent)
  } catch {
...
async function addAnimation(projectPath: string | undefined, args: Record<string, unknown>) {
  ...
  try {
    validateNoNewlines(undefined, animName)
  } catch {
    throw new GodotMCPError(
...
async function addTrack(projectPath: string | undefined, args: Record<string, unknown>) {
  ...
  try {
    validateNoNewlines(undefined, animName, trackType, nodePath, property)
  } catch {
    throw new GodotMCPError(
      'Invalid characters in parameters',
      'INVALID_ARGS',
...
export async function handleAnimation(action: string, args: Record<string, unknown>, config: GodotConfig) {
  ...
  switch (action) {
    case 'create_player':
      return createPlayer(projectPath, args)
    case 'add_animation':
      return addAnimation(projectPath, args)
    case 'add_track':
      return addTrack(projectPath, args)
    case 'add_keyframe':
      return addKeyframe()
    case 'list':
      return listAnimations(projectPath, args)
    default:
