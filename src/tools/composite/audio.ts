 ...
 */
function resolveBusLayoutPath(projectPath: string | null | undefined, _baseDir: string): string {
  // projectPath coming from resolveProjectRoot should be absolute string,
  // but we keep the check for safety as it was in original code.
  if (!projectPath) {
...
async function handleListBuses(projectPath: string, _args: Record<string, unknown>, config: GodotConfig) {
...
async function handleAddBus(projectPath: string, args: Record<string, unknown>, config: GodotConfig) {
  const baseDir = config.projectPath || process.cwd()
  const busLayoutPath = resolveBusLayoutPath(projectPath, baseDir)
...
async function handleAddEffect(projectPath: string, args: Record<string, unknown>, config: GodotConfig) {
  const baseDir = config.projectPath || process.cwd()
  const busLayoutPath = resolveBusLayoutPath(projectPath, baseDir)
  const busName = args.bus_name as string
...
async function handleCreateStream(projectPath: string, args: Record<string, unknown>, _config: GodotConfig) {
...
const AUDIO_ACTIONS: Record<
  string,
  (projectPath: string, args: Record<string, unknown>, config: GodotConfig) => Promise<ToolResult>
> = {
...
export async function handleAudio(action: string, args: Record<string, unknown>, config: GodotConfig) {
  const projectPathArg = args.project_path as string
  if (action === 'list_buses' && !projectPathArg && !config.projectPath) {
    throw new GodotMCPError('No project path specified', 'INVALID_ARGS', 'Provide project_path.')
  }

  const projectPath = resolveProjectRoot(projectPathArg, config.projectPath)

  const handler = AUDIO_ACTIONS[action]
  if (handler) {
    return handler(projectPath, args, config)
  }
