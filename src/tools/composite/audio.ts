...
/**
 * Lists all audio buses in the default bus layout.
 */
async function handleListBuses(projectPath: string | null | undefined, baseDir: string) {
...
/**
 * Adds a new audio bus to the default bus layout.
 */
async function handleAddBus(projectPath: string | null | undefined, baseDir: string, args: Record<string, unknown>) {
  const busLayoutPath = resolveBusLayoutPath(projectPath, baseDir)
...
/**
 * Adds an audio effect to an existing bus.
 */
async function handleAddEffect(projectPath: string | null | undefined, baseDir: string, args: Record<string, unknown>) {
  const busLayoutPath = resolveBusLayoutPath(projectPath, baseDir)
  const busName = args.bus_name as string
...
/**
 * Creates an AudioStreamPlayer node in a scene.
 */
async function handleCreateStream(
  ...
  const resolvedProjectPath = projectPath ? safeResolve(baseDir, projectPath) : baseDir
  const fullPath = safeResolve(resolvedProjectPath, scenePath)
...
export async function handleAudio(action: string, args: Record<string, unknown>, config: GodotConfig) {
  ...
  switch (action) {
    case 'list_buses':
      return handleListBuses(projectPath, baseDir)
    case 'add_bus':
      return handleAddBus(projectPath, baseDir, args)
    case 'add_effect':
      return handleAddEffect(projectPath, baseDir, args)
    case 'create_stream':
      return handleCreateStream(projectPath, baseDir, args)
    default:
