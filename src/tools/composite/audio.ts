...
/**
 * Helper to get the content of the bus layout file.
 * Returns default template if file does not exist.
 */
async function getBusLayoutContent(busLayoutPath: string): Promise<string> {
  if (await pathExists(busLayoutPath)) {
    return await readFile(busLayoutPath, 'utf-8')
  }
  return DEFAULT_BUS_LAYOUT
}

/**
 * List audio buses in the project.
 */
async function listBuses(projectPath: string) {
  const busLayoutPath = join(projectPath, 'default_bus_layout.tres')
...
/**
 * Add a new audio bus to the project.
 */
async function addBus(args: Record<string, unknown>, projectPath: string) {
  const busLayoutPath = join(projectPath, 'default_bus_layout.tres')
 ...
 */
async function addEffect(args: Record<string, unknown>, projectPath: string) {
  const busLayoutPath = join(projectPath, 'default_bus_layout.tres')
  const busName = args.bus_name as string
...
/**
 * Create an audio stream player node in a scene.
 */
async function createStream(args: Record<string, unknown>, projectPath: string) {
...
export async function handleAudio(action: string, args: Record<string, unknown>, config: GodotConfig) {
  ...
  switch (action) {
    case 'list_buses':
      return listBuses(projectPath)

    case 'add_bus':
      return addBus(args, projectPath)

    case 'add_effect':
      return addEffect(args, projectPath)

    case 'create_stream':
      return createStream(args, projectPath)
