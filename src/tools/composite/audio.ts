...
async function handleListBuses(projectPath: string | undefined, baseDir: string, _args: Record<string, unknown>) {
...
async function handleAddBus(projectPath: string | undefined, baseDir: string, args: Record<string, unknown>) {
  const busLayoutPath = resolveBusLayoutPath(projectPath, baseDir)
...
async function handleAddEffect(projectPath: string | undefined, baseDir: string, args: Record<string, unknown>) {
  const busLayoutPath = resolveBusLayoutPath(projectPath, baseDir)
  const busName = args.bus_name as string
...
async function handleCreateStream(args: Record<string, unknown>, config: GodotConfig) {
  ...
  const fullPath = safeResolve(resolveProjectRoot(args.project_path as string, config.projectPath), scenePath)
