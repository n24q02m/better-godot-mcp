  ...
  switch (action) {
    ...
    case 'add_event': {
      ...
      if (match) {
        ...
        updatedValue = actionValue.replace(eventsRegex, `$1${newEvents}$3`)
      } else {
        // If "events": [] missing, append it before the closing brace
        updatedValue = actionValue.replace(/\s*}$/, `,\n"events": [${eventObj}]\n}`)
        if (updatedValue === actionValue) {
