...
async function handleSetProperty(projectPath: string, args: Record<string, unknown>) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
  const { path: nodeName } = normalizeNodePath((args.name as string) || '')
...
async function handleGetProperty(projectPath: string, args: Record<string, unknown>) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
  const { path: nodeName } = normalizeNodePath((args.name as string) || '')
  const property = args.property as string
...
export async function handleNodes(action: string, args: Record<string, unknown>, config: GodotConfig) {
  ...
  switch (action) {
    ...
    case 'set_property':
      return handleSetProperty(projectPath, args)
    case 'get_property':
      return handleGetProperty(projectPath, args)
    default:
