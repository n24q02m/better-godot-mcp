...
async function handleAdd(args: Record<string, unknown>, projectPath: string) {
...
async function handleRemove(args: Record<string, unknown>, projectPath: string) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
  const nodeName = args.name as string
...
async function handleRename(args: Record<string, unknown>, projectPath: string) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
  const nodeName = args.name as string
...
async function handleList(args: Record<string, unknown>, projectPath: string) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
...
async function handleSetProperty(args: Record<string, unknown>, projectPath: string) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
  const nodeName = args.name as string
...
async function handleGetProperty(args: Record<string, unknown>, projectPath: string) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
  const nodeName = args.name as string
  const property = args.property as string
...
export async function handleNodes(action: string, args: Record<string, unknown>, config: GodotConfig) {
  ...
  switch (action) {
    case 'add':
      return handleAdd(args, projectPath as string)
    case 'remove':
      return handleRemove(args, projectPath as string)
    case 'rename':
      return handleRename(args, projectPath as string)
    case 'list':
      return handleList(args, projectPath as string)
    case 'set_property':
      return handleSetProperty(args, projectPath as string)
    case 'get_property':
      return handleGetProperty(args, projectPath as string)
    default:
      throw new GodotMCPError(
