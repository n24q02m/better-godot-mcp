...
async function handleLayers(projectPath: string, config: GodotConfig) {
...
async function handleCollisionSetup(projectPath: string, args: Record<string, unknown>, config: GodotConfig) {
...
async function handleBodyConfig(projectPath: string, args: Record<string, unknown>, config: GodotConfig) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
  const nodeName = args.name as string
  if (!nodeName) throw new GodotMCPError('No node name specified', 'INVALID_ARGS', 'Provide node name.')
...
async function handleSetLayerName(projectPath: string, args: Record<string, unknown>, config: GodotConfig) {
  if (!projectPath) throw new GodotMCPError('No project path specified', 'INVALID_ARGS', 'Provide project_path.')
...
const PHYSICS_ACTIONS: Record<
  string,
  (projectPath: string, args: Record<string, unknown>, config: GodotConfig) => Promise<ToolResult>
> = {
  layers: (projectPath, _args, config) => handleLayers(projectPath, config),
  collision_setup: handleCollisionSetup,
  body_config: handleBodyConfig,
  set_layer_name: handleSetLayerName,
}

export async function handlePhysics(action: string, args: Record<string, unknown>, config: GodotConfig) {
  const projectPath = (args.project_path as string) || config.projectPath || ''

  const handler = PHYSICS_ACTIONS[action]
  if (handler) {
    return handler(projectPath, args, config)
  }

  throwUnknownAction(action, Object.keys(PHYSICS_ACTIONS))
}
