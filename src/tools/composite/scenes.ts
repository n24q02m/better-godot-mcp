...
export async function handleScenes(action: string, args: Record<string, unknown>, config: GodotConfig) {
  ...
  switch (action) {
    case 'create': {
      ...
      const fullPath = safeResolve(resolve(projectPath), scenePath)
    ...
    case 'info': {
      ...
      const fullPath = safeResolve(basePath, scenePath)
      if (!existsSync(fullPath)) {
    ...
    case 'duplicate': {
      ...
      const basePath = projectPath ? resolve(projectPath) : process.cwd()
      const srcFull = safeResolve(basePath, scenePath)
      const dstFull = safeResolve(basePath, newPath)
