...
import { escapeRegExp, parseSceneContent } from '../helpers/scene-parser.js'
...
async function createControl(projectPath: string | null | undefined, args: Record<string, unknown>) {
...
async function setTheme(projectPath: string | null | undefined, args: Record<string, unknown>) {
...
async function layoutControl(projectPath: string | null | undefined, args: Record<string, unknown>) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
...
async function listControls(projectPath: string | null | undefined, args: Record<string, unknown>) {
  const scenePath = args.scene_path as string
  if (!scenePath) throw new GodotMCPError('No scene_path specified', 'INVALID_ARGS', 'Provide scene_path.')
...
export async function handleUI(action: string, args: Record<string, unknown>, config: GodotConfig) {
  ...
  switch (action) {
    case 'create_control':
      return await createControl(projectPath, args)
    case 'set_theme':
      return await setTheme(projectPath, args)
    case 'layout':
      return await layoutControl(projectPath, args)
    case 'list_controls':
      return await listControls(projectPath, args)
    default:
      throw new GodotMCPError(
